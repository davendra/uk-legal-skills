...
1. `~/.claude/skills/legal/scripts/` (where `install.sh` places the bundled PDF scripts)
...
constitute legal advice. Always consult a qualified solicitor.
...
☐ [Second action]
☐ [Third action]
☐ Consult a qualified solicitor before signing
```
...
ALWAYS consult a qualified solicitor before:
